...
const G = {
  page: {
    ...
    fontFamily: "'Space Grotesk', sans-serif", padding: 20, position: "relative",
  },
  orb1: { position:"fixed", top:"-12%", right:"-8%", width:520, height:520,
    background:"radial-gradient(circle, rgba(99,102,241,0.13) 0%, transparent 70%)",
    borderRadius:"50%", pointerEvents:"none" },
  orb2: { position:"fixed", bottom:"-15%", left:"-8%", width:440, height:440,
  ...
  logoBox: { width:36, height:36, background:"linear-gradient(135deg, #6366f1, #10b981)",
  ...
  input: { width:"100%", padding:"12px 16px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)",
    borderRadius:12, color:"#fff", fontSize:15, fontFamily:"'Plus Jakarta Sans', sans-serif", outline:"none",
    transition:"border-color 0.2s, box-shadow 0.2s", marginBottom:20, display:"block" },
  btn: { width:"100%", padding:"14px", background:"linear-gradient(135deg, #6366f1, #8b5cf6)",
    border:"none", color:"#fff", borderRadius:12, fontSize:16, fontWeight:800,
    cursor:"pointer", fontFamily:"'Space Grotesk', sans-serif", letterSpacing:"0.2px",
    boxShadow:"0 4px 22px rgba(99,102,241,0.42)", transition:"all 0.2s", marginTop:4 },
  err: { background:"rgba(239,68,68,0.09)", border:"1px solid rgba(239,68,68,0.3)",
  ...
  link: { background:"none", border:"none", color:"#818cf8", cursor:"pointer", fontWeight:700, fontSize:14, fontFamily:"'Plus Jakarta Sans', sans-serif" },
  back: { position:"fixed", top:24, left:28, background:"none", border:"none",
    color:"rgba(255,255,255,0.28)", cursor:"pointer", fontSize:13,
    fontFamily:"'Plus Jakarta Sans', sans-serif", display:"flex", alignItems:"center", gap:4, zIndex:20 },
};
...
export default function Login({ nav, login }) {
  ...
  const submit = async () => {
    ...
    try {
      const r = await fetch("https://loan-ml-production.up.railway.app/api/auth/login", {
        method: "POST",
  ...
  return (
    <div style={G.page}>
      <div style={G.orb1} /><div style={G.orb2} />
      <button onClick={() => nav("landing")} style={G.back}>← Home</button>

      <div style={G.card}>
        <div style={G.logo}>
          <div style={G.logoBox}>L</div>
          <span style={{ fontSize:20, fontWeight:900, color:"#fff", letterSpacing:"-0.5px" }}>LoanPredict</span>
        </div>
        <h1 style={G.h1}>Welcome back</h1>
        <p style={G.sub}>Sign in to your account</p>

        {error && <div style={G.err}>{error}</div>}

        <label style={G.label}>Email</label>
        <input value={email} onChange={e => setEmail(e.target.value)} type="email"
          placeholder="you@email.com" style={G.input}
          onFocus={focusIn} onBlur={focusOut}
        />

        <label style={G.label}>Password</label>
        <input value={password} onChange={e => setPassword(e.target.value)} type="password"
          placeholder="••••••••" style={{ ...G.input, marginBottom:0 }}
          onFocus={focusIn} onBlur={focusOut}
          onKeyDown={e => e.key === "Enter" && submit()}
        />

        <button onClick={submit} disabled={loading} style={{ ...G.btn, opacity: loading ? 0.65 : 1, marginTop:24 }}
        ...
        <div style={G.foot}>
          ...
          <button onClick={() => nav("register")} style={G.link}>Create one</button>
        </div>
      </div>
